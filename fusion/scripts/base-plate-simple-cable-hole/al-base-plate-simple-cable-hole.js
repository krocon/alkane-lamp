...
function setupParameters(design) {
    ...
    return {
        ...
        cableHoleHeight: getOrCreateParam('cable_hole_height', '4.5mm', 'mm', 'Höhe des Kabelkanallochs über der Unterseite (8.0mm)'),
 ...
 */
function createSimpleCableHole(rootComp, params, _body) {
    ...
    sketch.sketchCurves.sketchCircles.addByCenterRadius(centerPoint, holeRadius);
    if (sketch.profiles.count === 0) {
        throw new Error('Profil für das Kabelkanal-Loch nicht gefunden.');
    }
    const profile = sketch.profiles.item(0);
    if (!profile) {
    ...
    if (chamferVal > 0 && cutFeature.sideFaces) {
        ...
        if (chamferEdges.length > 0) {
            ...
            if (chamferInput) {
                ...
                chamferInput.chamferEdgeSets.addEqualDistanceChamferEdgeSet(edgeColl, adsk.core.ValueInput.createByString('cable_hole_chamfer'), true);
