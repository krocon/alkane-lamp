...
function setupParameters(design: adsk.fusion.Design) {
  ...
  return {
    ...
    stopRingWidth: getOrCreateParam('stop_ring_width', '2mm', 'mm', 'Breite des Anschlagrings in der Mitte (0mm = deaktiviert)'),
    stopRingDiameter: getOrCreateParam('stop_ring_diameter', '38.5mm', 'mm', 'Außendurchmesser des Anschlagrings in der Mitte'),
...
function createConnectorPipe(
  ...
  planeInput.setByOffset(
    ...
    adsk.core.ValueInput.createByReal(-halfLength)
  );
  ...
  const points: adsk.core.Point3D[] = [];
  ...
  extInput.setDistanceExtent(
    false,
    adsk.core.ValueInput.createByReal(totalLength)
  );
  ...
  if (stopWidth > 0 && rStop > rInner) {
    ...
    stopPlaneInput.setByOffset(
      rootComp.xYConstructionPlane,
      adsk.core.ValueInput.createByReal(-halfStopWidth)
    );
    ...
    let stopRingProfile: adsk.fusion.Profile | null = null;
    ...
    if (stopRingProfile) {
      const extStopInput = extrudeFeatures.createInput(
        stopRingProfile,
        adsk.fusion.FeatureOperations.JoinFeatureOperation
      );
      extStopInput.setDistanceExtent(
        false,
        adsk.core.ValueInput.createByReal(stopWidth)
      );
  ...
  if (chamfer > 0) {
    ...
    if (topSketch.profiles.count > 0) {
      ...
      const topRevInput = revolveFeatures.createInput(
        topProfile,
    ...
    if (botSketch.profiles.count > 0) {
      ...
      const botRevInput = revolveFeatures.createInput(
        botProfile,
        rootComp.zConstructionAxis,
        adsk.fusion.FeatureOperations.CutFeatureOperation
      );
