 ...
 */
function createOffsetPlane(rootComp, basePlane, offsetCm) {
 ...
 */
function getLiveBody(rootComp, fallbackBody) {
 ...
 */
function findRingProfile(sketch, innerDiameterCm) {
 ...
 */
function findInnerAndOuterProfiles(sketch) {
    let innerProfile = null;
    let outerRingProfile = null;
    for (let i = 0; i < sketch.profiles.count; i++) {
        const prof = sketch.profiles.item(i);
 ...
 */
function applyFilletWithFallbacks(rootComp, edges, radiusCm, paramName, logPrefix = 'Fillet') {
    if (edges.length === 0) {
    ...
    if (paramName) {
        try {
            ...
            if (input) {
                ...
                if (!valInput)
                    valInput = adsk.core.ValueInput.createByReal(radiusCm);
                const setInput = input.edgeSetInputs.addConstantRadiusEdgeSet(coll, valInput, true);
                if (setInput)
                    setInput.continuity = 1 /* adsk.fusion.SurfaceContinuityTypes.TangentSurfaceContinuityType */;
 ...
 */
function cutInnerSphere(rootComp, innerBallDiameterParam) {
    ...
    const arc = sketch.sketchCurves.sketchArcs.addByCenterStartSweep(center, startPoint, Math.PI);
    // Schließlinie durch die Endpunkte des Bogens zeichnen
    sketch.sketchCurves.sketchLines.addByTwoPoints(arc.geometry.startPoint, arc.geometry.endPoint);
    ...
    const revolveInput = revolveFeatures.createInput(profile, rootComp.yConstructionAxis, 1 /* adsk.fusion.FeatureOperations.CutFeatureOperation */);
 ...
 */
function createLongArm(rootComp, params) {
    const sketches = rootComp.sketches;
    ...
    const extInputRing = extrudeFeatures.createInput(outerRingProfile, 3 /* adsk.fusion.FeatureOperations.NewBodyFeatureOperation */);
 ...
 */
function createSingleSteppedArm(rootComp, params) {
    const sketches = rootComp.sketches;
    const features = rootComp.features;
    const extrudeFeatures = features.extrudeFeatures;
    const xyPlane = rootComp.xYConstructionPlane;
    ...
    const extInputInner = extrudeFeatures.createInput(innerProfile, 3 /* adsk.fusion.FeatureOperations.NewBodyFeatureOperation */);
    ...
    combineInput.operation = 0 /* adsk.fusion.FeatureOperations.JoinFeatureOperation */;
 ...
 */
function createTetrapod(rootComp, params, armTypes) {
    const features = rootComp.features;
...
function createArmBody(rootComp, params, armType) {
    if (armType === 'leg') {
 ...
 */
function createSingleThreadedArm(rootComp, params) {
...
function addShortArmHoles(rootComp, armBody, holeDiameterParam) {
    const sketches = rootComp.sketches;
    const features = rootComp.features;
    ...
    for (let i = 0; i < armBody.faces.count; i++) {
        ...
        if (face.geometry.surfaceType === 0 /* adsk.core.SurfaceTypes.PlaneSurfaceType */) {
            const bbox = face.boundingBox;
            const faceCenter = adsk.core.Point3D.create((bbox.minPoint.x + bbox.maxPoint.x) / 2, (bbox.minPoint.y + bbox.maxPoint.y) / 2, (bbox.minPoint.z + bbox.maxPoint.z) / 2);
    ...
    for (const face of targetFaces) {
        ...
        sketch.sketchCurves.sketchCircles.addByCenterRadius(adsk.core.Point3D.create(0, 0, 0), holeDiameterParam.value / 2.0);
        if (sketch.profiles.count === 0)
            continue;
        ...
        const extrudeFeatures = features.extrudeFeatures;
        const extInput = extrudeFeatures.createInput(holeProfile, 1 /* adsk.fusion.FeatureOperations.CutFeatureOperation */);
...
function addLongArmThread(rootComp, armBody, params) {
    const features = rootComp.features;
    ...
    for (let i = 0; i < armBody.faces.count; i++) {
        const face = armBody.faces.item(i);
        if (face.geometry.surfaceType === 1 /* adsk.core.SurfaceTypes.CylinderSurfaceType */) {
            const cyl = face.geometry;
    ...
    if (!targetFace) {
        if (ui)
            ui.messageBox("Konnte die Innenfläche des langen Arms für das Gewinde nicht finden.");
        return;
    }
    ...
    if (!threadFeature) {
        if (ui)
            ui.messageBox("Fehler beim Erstellen des Gewinde-Features.");
        return;
    }
    ...
    if (facesToOffset.length > 0) {
        ...
        const offsetInput = offsetFeatures.createInput(facesToOffset, adsk.core.ValueInput.createByString("-0.15mm"));
        if (offsetInput) {
...
function boreOutLongArm(rootComp, armBody, _params) {
    ...
    sketch.sketchCurves.sketchCircles.addByCenterRadius(adsk.core.Point3D.create(0, 0, 0), 4.1 / 2.0 // 41mm Durchmesser (Radius 2.05 cm)
    );
    if (sketch.profiles.count === 0)
        return;
    const profile = sketch.profiles.item(0);
    const extrudeFeatures = rootComp.features.extrudeFeatures;
    const extInput = extrudeFeatures.createInput(profile, 1 /* adsk.fusion.FeatureOperations.CutFeatureOperation */);
...
function boreOutFromOrigin(rootComp, armBody) {
    const sketch = rootComp.sketches.add(rootComp.xYConstructionPlane);
    sketch.sketchCurves.sketchCircles.addByCenterRadius(adsk.core.Point3D.create(0, 0, 0), 4.005 / 2.0 // 40.025mm Durchmesser (Radius 2.0025 cm)
    );
    if (sketch.profiles.count === 0)
        return;
    const profile = sketch.profiles.item(0);
...
function positionSecondTetrapod(rootComp, node2) {
    const dirX = -Math.sin(TETRA_ANGLE_RAD);
    const dirY = 0.0;
    const dirZ = -Math.cos(TETRA_ANGLE_RAD);
    const distanceCm = 8.0;
    const center2 = adsk.core.Point3D.create(distanceCm * dirX, dirY, distanceCm * dirZ);
...
function positionThirdTetrapod(rootComp, node3, center2) {
    const distanceCm = 8.0;
    const center3 = adsk.core.Point3D.create(center2.x, 0, center2.z + distanceCm);
    const transformMatrix = adsk.core.Matrix3D.create();
...
function createTubeBody(rootComp, params, lengthCm, negativeDirection = false) {
    const sketches = rootComp.sketches;
    const features = rootComp.features;
    const extrudeFeatures = features.extrudeFeatures;
    const sketch = sketches.add(rootComp.xYConstructionPlane);
...
function createConnectionTube1To2(rootComp, params, node1, node2) {
...
function createConnectionTube2To3(rootComp, params, node1, node3, center2) {
    const tubeLengthCm = 4.5;
    const tubeBody = createTubeBody(rootComp, params, tubeLengthCm, false);
    const offsetCm = 1.75;
    const transformMatrix = adsk.core.Matrix3D.create();
    transformMatrix.translation = adsk.core.Vector3D.create(center2.x, 0, center2.z + offsetCm);
    const moveFeats = rootComp.features.moveFeatures;
    const moveInput = moveFeats.createInput2(createCollection([tubeBody]));
    moveInput.defineAsFreeMove(transformMatrix);
    moveFeats.add(moveInput);
 ...
 */
function rotateFootBySplittingLeg(rootComp, targetBody) {
...
function combineAllBodies(rootComp, targetBody) {
 ...
 */
function alignFootToXYPlane(rootComp, body) {
    // 1. Unterste ebene Fläche finden
    let footBottomFace = null;
    let minZ = Infinity;
    ...
    if (!normal) {
        const planeGeom = footBottomFace.geometry;
...
function createVerticalLegForNode1(rootComp, params) {
    const sketches = rootComp.sketches;
    const features = rootComp.features;
    const extrudeFeatures = features.extrudeFeatures;
    const xyPlane = rootComp.xYConstructionPlane;
...
function createTiltedBasePlateFoot(rootComp, params, node1) {
    const sketches = rootComp.sketches;
    const features = rootComp.features;
    ...
    const extInput1 = features.extrudeFeatures.createInput(profile1, 3 /* adsk.fusion.FeatureOperations.NewBodyFeatureOperation */);
    ...
    if (!plateFeat || plateFeat.bodies.count === 0)
        return node1;
    ...
    let trimToolBody = null;
    if (sketch2.profiles.count > 0) {
        const extInput2 = features.extrudeFeatures.createInput(sketch2.profiles.item(0), 3 /* adsk.fusion.FeatureOperations.NewBodyFeatureOperation */);
    ...
    let cableToolBody = null;
    const plateRadiusCm = params.basePlateDiameter.value / 2.0;
    const cableOffsetVal = Math.max(-offsetCm + params.cableHoleOffset.value, -offsetCm + plateRadiusCm + 1.0);
    ...
    if (cableSketch.profiles.count > 0) {
        const cableExtInput = features.extrudeFeatures.createInput(cableSketch.profiles.item(0), 3 /* adsk.fusion.FeatureOperations.NewBodyFeatureOperation */);
    ...
    const transformMatrix = adsk.core.Matrix3D.create();
    transformMatrix.setToRotation(rotAngleRad, adsk.core.Vector3D.create(0, 1, 0), adsk.core.Point3D.create(0, 0, plateTopZ));
    const moveFeats = features.moveFeatures;
    ...
    if (cableToolBody) {
        ...
        cableCutInput.operation = 1 /* adsk.fusion.FeatureOperations.CutFeatureOperation */;
        try {
    ...
    if (trimToolBody) {
        ...
        cutInput.operation = 1 /* adsk.fusion.FeatureOperations.CutFeatureOperation */;
        try {
...
function boreVerticalLegHole(rootComp, params, node1) {
    const sketches = rootComp.sketches;
    const features = rootComp.features;
    const extrudeFeatures = features.extrudeFeatures;
    const sketch = sketches.add(rootComp.xYConstructionPlane);
    ...
    if (sketch.profiles.count === 0)
        return;
    const profile = sketch.profiles.item(0);
    const cutInput = extrudeFeatures.createInput(profile, 1 /* adsk.fusion.FeatureOperations.CutFeatureOperation */);
...
function filletBasePlateTopEdgeAtEnd(rootComp, body, params) {
    ...
    for (let i = 0; i < body.edges.count; i++) {
        ...
        if (geom) {
            if (geom.curveType === 2 /* adsk.core.Curve3DTypes.Circle3DCurveType */) {
                const circle = geom;
                radius = circle.radius;
                centerZ = circle.center.z;
            }
            else if (geom.curveType === 1 /* adsk.core.Curve3DTypes.Arc3DCurveType */) {
                const arc = geom;
                radius = arc.radius;
    ...
    if (topEdge) {
        applyFilletWithFallbacks(rootComp, [topEdge], params.basePlateRounding.value, 'base_plate_rounding', 'Step 9 (Basis-Platte Obere Kante)');
    }
    else {
...
function filletLegPlateJunction(rootComp, body, topZ, legRadius, _params) {
    ...
    if (edges.length > 0) {
        applyFilletWithFallbacks(rootComp, edges, 0.4, // 4mm
        'leg_plate_rounding', 'Leg-Plate Junction');
    }
}
/** Bringt eine Abfasung (0.7mm) an den 2 Kabelkanal-Öffnungskanten an. */
function chamferCableHoleOpenings(rootComp, params, body) {
    ...
    for (let i = 0; i < body.edges.count; i++) {
        ...
        for (let f = 0; f < edge.faces.count; f++) {
            ...
            if (face && face.geometry.surfaceType === 1 /* adsk.core.SurfaceTypes.CylinderSurfaceType */) {
                const cyl = face.geometry;
    ...
    if (chamferEdges.length > 0) {
        ...
        try {
            ...
            try {
                ...
                fallbackInput.chamferEdgeSets.addEqualDistanceChamferEdgeSet(edgeColl, adsk.core.ValueInput.createByReal(chamferVal), true);
...
function trimBottomFlushAtZ(rootComp, body, plateBottomZ, baseDiameterCm) {
...
function isOuterTetrapodIntersectionEdgeByLength(edge) {
...
function findNodeIntersectionEdges(targetBody, center) {
    const candidates = [];
...
function applyNodeFilletsAtEnd(rootComp, targetBody, params, center2, center3) {
    ...
    for (const item of nodeCenters) {
        const edges = findNodeIntersectionEdges(liveBody, item.center);
        if (edges.length > 0) {
            applyFilletWithFallbacks(rootComp, edges, params.nodeFilletRadius.value, 'node_fillet_radius', `Step 10 (${item.name})`);
        }
    }
...
function filletTubeEdges(rootComp, targetBody) {
    const liveBody = getLiveBody(rootComp, targetBody);
    const matchingEdges = [];
    ...
    applyFilletWithFallbacks(rootComp, matchingEdges, 1.0, // 10mm = 1.0 cm
    undefined, 'Step 12 (Röhren-Kanten)');
}
