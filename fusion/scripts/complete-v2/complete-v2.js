...
function boreOutLongArm(rootComp, armBody, _params) {
    ...
    sketch.sketchCurves.sketchCircles.addByCenterRadius(adsk.core.Point3D.create(0, 0, 0), diameterCm / 2.0);
    if (sketch.profiles.count === 0)
        return;
    const profile = sketch.profiles.item(0);
...
function filletBasePlateTopEdgeAtEnd(rootComp, body, params) {
    ...
    if (topEdge) {
        ...
        filletInput.edgeSetInputs.addConstantRadiusEdgeSet(edgeColl, valInput, false);
        try {
            ...
            try {
                ...
                fallbackInput.edgeSetInputs.addConstantRadiusEdgeSet(fallbackColl, adsk.core.ValueInput.createByReal(params.basePlateRounding.value), false);
...
function filletLegPlateJunction(rootComp, body, topZ, legRadius, _params) {
    ...
    if (edges.length > 0) {
        ...
        filletInput.edgeSetInputs.addConstantRadiusEdgeSet(edgeColl, adsk.core.ValueInput.createByString('leg_plate_rounding'), false);
        try {
 ...
 */
function applyFilletToEdgeGroup(rootComp, edges, params, nodeName) {
    if (edges.length === 0) {
    ...
    try {
        ...
        if (input1) {
            ...
            if (!valInput)
                valInput = adsk.core.ValueInput.createByReal(radiusParamVal);
            const set1 = input1.edgeSetInputs.addConstantRadiusEdgeSet(coll1, valInput, true);
            if (set1) {
                set1.continuity = 1 /* adsk.fusion.SurfaceContinuityTypes.TangentSurfaceContinuityType */;
            }
    ...
    try {
        ...
        if (input2) {
            ...
            if (set2) {
                set2.continuity = 1 /* adsk.fusion.SurfaceContinuityTypes.TangentSurfaceContinuityType */;
            }
    ...
    try {
        ...
        if (input3) {
            ...
            if (set3) {
                set3.continuity = 1 /* adsk.fusion.SurfaceContinuityTypes.TangentSurfaceContinuityType */;
            }
    ...
    for (const edge of edges) {
        try {
            ...
            if (input4) {
                ...
                if (set4) {
                    set4.continuity = 1 /* adsk.fusion.SurfaceContinuityTypes.TangentSurfaceContinuityType */;
                }
