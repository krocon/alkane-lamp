 ...
 */
function createBasePlate(rootComp, params) {
    ...
    const extInput = rootComp.features.extrudeFeatures.createInput(profile, 3 /* adsk.fusion.FeatureOperations.NewBodyFeatureOperation */);
    if (!extInput) {
    ...
    filletInput.edgeSetInputs.addConstantRadiusEdgeSet(edgeColl, adsk.core.ValueInput.createByString('base_plate_rounding'), false);
 ...
 */
function findCircularEdgeAtZ(body, z, radius) {
 ...
 */
function createLegAxis(rootComp, params) {
 ...
 */
function createTiltedConstructionPlane(rootComp, legAxis) {
 ...
 */
function createLegTube(rootComp, params, tiltedPlane, legAxis, baseBody) {
 ...
 */
function extrudeAlongAxisDown(rootComp, sketch, params) {
    ...
    const strategies = [
        // Strategie 1: Positiv-String 'leg_length + base_plate_height' (entlang Normalenvektor nach unten)
        {
            ...
            run: () => {
                const input = extrudeFeatures.createInput(profileArg, 3 /* adsk.fusion.FeatureOperations.NewBodyFeatureOperation */);
        ...
        // Strategie 2: setOneSideExtent mit PositiveExtentDirection und positivem String
        {
            ...
            run: () => {
                ...
                if (input && distVal) {
                    ...
                    if (distDef) {
                        input.setOneSideExtent(distDef, 0 /* adsk.fusion.ExtentDirections.PositiveExtentDirection */);
                        return extrudeFeatures.add(input);
                    }
                }
                return null;
        ...
        // Strategie 4: setOneSideExtent mit NegativeExtentDirection und positivem String
        {
            ...
            run: () => {
                ...
                if (input && distVal) {
                    const distDef = adsk.fusion.DistanceExtentDefinition.create(distVal);
                    if (distDef) {
                        input.setOneSideExtent(distDef, 1 /* adsk.fusion.ExtentDirections.NegativeExtentDirection */);
                        return extrudeFeatures.add(input);
                    }
                }
                return null;
 ...
 */
function joinBodies(rootComp, target, tools) {
 ...
 */
function cutStepShoulder(rootComp, params, sketch, body, legAxis) {
    const innerRadius = params.ringInnerDiameter.value / 2.0;
    const extrudeFeatures = rootComp.features.extrudeFeatures;
    const errors = [];
    ...
    const strategies = [
        () => {
            const input = extrudeFeatures.createInput(ringProfile, 1 /* adsk.fusion.FeatureOperations.CutFeatureOperation */);
 ...
 * abweichen kann.)
 */
function findMaxAreaProfile(profiles) {
    let best = null;
    let bestArea = -1;
 ...
 */
function hasCylinderOfRadius(body, radius, axisDir) {
 ...
 */
function filletLegPlateJunction(rootComp, params, body, legAxis) {
 ...
 */
function applyConstantRadiusFillet(rootComp, edges, radiusExpr) {
    const filletInput = rootComp.features.filletFeatures.createInput();
    if (!filletInput) {
    ...
    filletInput.edgeSetInputs.addConstantRadiusEdgeSet(edgeColl, adsk.core.ValueInput.createByString(radiusExpr), false);
    const filletFeature = rootComp.features.filletFeatures.add(filletInput);
    if (!filletFeature) {
        return false;
    }
 ...
 */
function radialDistance(p, legAxis) {
 ...
 */
function boreLegHole(rootComp, params, tiltedPlane, body, legAxis) {
    ...
    const strategies = [
        () => {
            const input = extrudeFeatures.createInput(profile, 1 /* adsk.fusion.FeatureOperations.CutFeatureOperation */);
            const dist = adsk.core.ValueInput.createByString('-leg_length - base_plate_height');
            if (input && dist) {
                input.setDistanceExtent(false, dist);
                return extrudeFeatures.add(input);
 ...
 */
function trimBottomFlush(rootComp, params) {
    const sketch = rootComp.sketches.add(rootComp.xYConstructionPlane);
    if (!sketch) {
    ...
    if (distVal) {
        const distDef = adsk.fusion.DistanceExtentDefinition.create(distVal);
        if (distDef) {
            cutInput.setOneSideExtent(distDef, 1 /* adsk.fusion.ExtentDirections.NegativeExtentDirection */);
            try {
    ...
    if (fallbackDist) {
        const fallbackInput = extrudeFeatures.createInput(profile, 1 /* adsk.fusion.FeatureOperations.CutFeatureOperation */);
        if (fallbackInput) {
 ...
 */
function createBottomCableChannel(rootComp, params) {
    const extrudeFeatures = rootComp.features.extrudeFeatures;
    ...
    const sweepInput = sweepFeatures.createInput(profile, path, 1 /* adsk.fusion.FeatureOperations.CutFeatureOperation */);
    if (!sweepInput) {
 ...
 */
function drawAnnularSector(sketch, center, rIn, rOut, startAngle, sweepAngle) {
 ...
 */
function createBottomCoverPlate(rootComp, params) {
    const extrudeFeatures = rootComp.features.extrudeFeatures;
    ...
    const recessCutInput = extrudeFeatures.createInput(recessProfile, 1 /* adsk.fusion.FeatureOperations.CutFeatureOperation */);
    ...
    if (entrySketch.profiles.count > 0) {
        ...
        if (entryProfColl) {
            ...
            const entryCutInput = extrudeFeatures.createInput(entryProfColl, 1 /* adsk.fusion.FeatureOperations.CutFeatureOperation */);
    ...
    if (slotSketch.profiles.count > 0) {
        ...
        if (slotProfColl) {
            ...
            const slotCutInput = extrudeFeatures.createInput(slotProfColl, 1 /* adsk.fusion.FeatureOperations.CutFeatureOperation */);
    ...
    const plateExtInput = extrudeFeatures.createInput(plateProf, 3 /* adsk.fusion.FeatureOperations.NewBodyFeatureOperation */);
    ...
    if (tabSketch.profiles.count > 0) {
        ...
        if (tabProfColl) {
            ...
            const tabExtInput = extrudeFeatures.createInput(tabProfColl, 3 /* adsk.fusion.FeatureOperations.NewBodyFeatureOperation */);
    ...
    if (gripSketch) {
        ...
        if (gripSketch.profiles.count > 0) {
            ...
            if (gripProfColl) {
                ...
                const gripCutInput = extrudeFeatures.createInput(gripProfColl, 1 /* adsk.fusion.FeatureOperations.CutFeatureOperation */);
