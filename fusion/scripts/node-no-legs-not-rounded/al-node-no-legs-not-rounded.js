 ...
 */
function createValueInput(valStr) {
 ...
 */
function createSingleArm(rootComp, params) {
    ...
    for (let i = 0; i < sketch.profiles.count; i++) {
        ...
        if (!minPt || !maxPt)
            continue;
        const maxDist = Math.max(minPt.distanceTo(center), maxPt.distanceTo(center));
    ...
    const extInputOuter = extrudeFeatures.createInput(outerRingProfile, 3 /* adsk.fusion.FeatureOperations.NewBodyFeatureOperation */);
    if (!extInputOuter) {
    ...
    const extInputMiddle = extrudeFeatures.createInput(middleRingProfile, 3 /* adsk.fusion.FeatureOperations.NewBodyFeatureOperation */);
    if (!extInputMiddle) {
 ...
 * @param params Die konfigurierten Benutzerparameter.
 */
function boreArmHoles(rootComp, targetBody, params) {
    const sketches = rootComp.sketches;
    const features = rootComp.features;
    const extrudeFeatures = features.extrudeFeatures;
    const xyPlane = rootComp.xYConstructionPlane;
    ...
    sketch0.sketchCurves.sketchCircles.addByCenterRadius(center, params.holeDiameter.value / 2.0);
    ...
    const extInput0 = extrudeFeatures.createInput(prof0, 3 /* adsk.fusion.FeatureOperations.NewBodyFeatureOperation */);
    if (!extInput0) {
    ...
    sketch1.sketchCurves.sketchCircles.addByCenterRadius(center, params.holeDiameter.value / 2.0);
    ...
    const extInput1 = extrudeFeatures.createInput(prof1, 3 /* adsk.fusion.FeatureOperations.NewBodyFeatureOperation */);
    if (!extInput1) {
    ...
    combineInput.operation = 1 /* adsk.fusion.FeatureOperations.CutFeatureOperation */;
    const combineFeat = combineFeatures.add(combineInput);
    if (!combineFeat) {
 ...
 * @param params Die konfigurierten Benutzerparameter.
 */
function filletOuterArmEdges(rootComp, targetBody, params) {
    ...
    for (let i = 0; i < targetBody.edges.count; i++) {
        ...
        if (!g1 || !g2)
            continue;
        if (g1.surfaceType === 1 /* adsk.core.SurfaceTypes.CylinderSurfaceType */ &&
            g2.surfaceType === 1 /* adsk.core.SurfaceTypes.CylinderSurfaceType */) {
            const cyl1 = g1;
            const cyl2 = g2;
