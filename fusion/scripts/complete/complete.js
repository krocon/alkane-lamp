...
function applyNodeFilletsAtEnd(rootComp, targetBody, params, center2, center3) {
    ...
    const constantEdgeSet = filletInput.edgeSetInputs.addConstantRadiusEdgeSet(edgeCollection, valInput, true);
    if (!constantEdgeSet)
        return;
    // Kontinuitätstyp: Tangential (G1)
    constantEdgeSet.continuity = 1 /* adsk.fusion.SurfaceContinuityTypes.TangentSurfaceContinuityType */;
    try {
        ...
        try {
            ...
            const fallbackEdgeSet = fallbackInput.edgeSetInputs.addConstantRadiusEdgeSet(edgeCollection, adsk.core.ValueInput.createByReal(params.nodeFilletRadius.value), true);
            if (fallbackEdgeSet) {
                fallbackEdgeSet.continuity = 1 /* adsk.fusion.SurfaceContinuityTypes.TangentSurfaceContinuityType */;
            }
            filletFeatures.add(fallbackInput);
