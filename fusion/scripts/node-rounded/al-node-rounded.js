 ...
 */
function createBaseTetrapod(rootComp, params) {
    ...
    if (!combFeat || combFeat.bodies.count === 0)
        return null;
 ...
 */
function applyNodeFillets(rootComp, targetBody, params) {
    ...
    if (!chordLengthVal)
        return;
    const chordEdgeSet = filletInput.edgeSetInputs.addChordLengthEdgeSet(edgeCollection, chordLengthVal, true);
    if (!chordEdgeSet)
        return;
    // Kontinuitätstyp: Tangential
    chordEdgeSet.continuity = 1 /* adsk.fusion.SurfaceContinuityTypes.TangentSurfaceContinuityType */;
    // Tangentialitätsgewicht: 1.0
 ...
 * @param params Die Benutzerparameter.
 */
function createArmStepsAndHoles(rootComp, targetBody, params) {
    const center = adsk.core.Point3D.create(0, 0, 0);
    if (!center)
        return;
    ...
    for (let i = 0; i < targetBody.faces.count; i++) {
        ...
        if (face && face.geometry.surfaceType === 0 /* adsk.core.SurfaceTypes.PlaneSurfaceType */) {
            const bbox = face.boundingBox;
            if (bbox && bbox.minPoint && bbox.maxPoint) {
                const faceCenter = adsk.core.Point3D.create((bbox.minPoint.x + bbox.maxPoint.x) / 2.0, (bbox.minPoint.y + bbox.maxPoint.y) / 2.0, (bbox.minPoint.z + bbox.maxPoint.z) / 2.0);
    ...
    for (const face of endFaces) {
        ...
        if (holeProfile) {
            const extHoleInput = extrudeFeatures.createInput(holeProfile, 1 /* adsk.fusion.FeatureOperations.CutFeatureOperation */);
        ...
        if (outerRingProfile) {
            const extRingInput = extrudeFeatures.createInput(outerRingProfile, 1 /* adsk.fusion.FeatureOperations.CutFeatureOperation */);
